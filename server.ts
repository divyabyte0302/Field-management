...
} from './server/slaEngine';
import { OPENAPI_SPEC, renderSwaggerHtml } from './server/openapi';
...
async function startServer() {
  ...
  app.use((req, res, next) => {
    const rawOrigins = process.env.CORS_ALLOWED_ORIGINS || process.env.CORS_ORIGINS || 'http://localhost:3000,http://localhost:5173';
  ...
  // Centralized Error Handling Middleware (No stack traces exposed in production)
  ...
  // ---------------------------------------------------------------------------
  // 8. VITE MIDDLEWARE (Production / Development SPA routing)
  // ---------------------------------------------------------------------------
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`[KEYSTONE FSM] Running on http://localhost:${PORT}`);
  });
}
