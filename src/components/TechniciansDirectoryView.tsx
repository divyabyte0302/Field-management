      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-5">
        ...
        {onRefreshData && (
          <button
            ...
            className="flex items-center gap-2 px-3.5 py-1.5 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 text-xs font-semibold shadow-xs transition"
          >
          ...
          return (
            ...
            >
              <div>
                ...
                <div className="space-y-1 text-xs text-slate-600 mb-4 bg-slate-50 border border-slate-200 rounded-lg p-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-slate-700 truncate">{tech.email}</span>
                  </div>
                </div>
                ...
                <div className="mb-4">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {tech.skills?.map(skill => (
                      <span key={skill} className="px-2 py-0.5 rounded text-[11px] bg-slate-50 text-slate-700 border border-slate-200 font-medium">
                        {skill}
                      </span>
                    ))}
                  </div>
              ...
              <div className="border-t border-slate-100 pt-3 flex items-center justify-between text-xs">
                {activeWO ? (
                  <div className="truncate pr-2">
                    <span className="text-[10px] text-blue-600 font-semibold block">CURRENT ACTIVE JOB</span>
                    <span className="text-slate-800 truncate block font-medium">{activeWO.workOrderNumber}: {activeWO.title}</span>
                  </div>
                ) : (
                ...
                <span className="text-blue-600 font-semibold text-xs group-hover:underline shrink-0">
                  Profile &rarr;
                </span>
              </div>
            </div>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6 space-y-5 shadow-xl text-slate-900">
            <div className="flex items-start justify-between border-b border-slate-100 pb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-bold text-slate-900">{activeTechDetail.name}</h2>
            ...
            <div className="grid grid-cols-4 gap-3 text-center">
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-3">
                ...
                <div className="text-base font-bold text-slate-900 mt-1">{activeTechDetail.totalJobsAssigned ?? 12}</div>
              </div>
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-3">
                <span className="text-[10px] text-slate-500 uppercase font-semibold">Completed</span>
                <div className="text-base font-bold text-emerald-600 mt-1">{activeTechDetail.completedJobsCount ?? 10}</div>
              </div>
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-3">
                <span className="text-[10px] text-slate-500 uppercase font-semibold">Rating</span>
                <div className="text-base font-bold text-amber-500 mt-1 flex items-center justify-center gap-1">
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-3">
              <div>
                <h4 className="text-[10px] text-slate-500 uppercase font-semibold tracking-wider mb-1.5">Trade Qualifications & Skills</h4>
                <div className="flex flex-wrap gap-1.5">
                  {activeTechDetail.skills?.map((s: string) => (
                    <span key={s} className="px-2.5 py-1 rounded-md text-xs bg-white border border-slate-200 text-slate-800 font-medium">
                      {s}
                    </span>
                  ))}
                </div>
              ...
              <div>
                <h4 className="text-[10px] text-slate-500 uppercase font-semibold tracking-wider mb-1.5">Active Certifications</h4>
                <div className="flex flex-wrap gap-1.5">
                  {activeTechDetail.certifications?.map((c: string) => (
                    <span key={c} className="px-2.5 py-1 rounded-md text-xs bg-blue-50 border border-blue-200 text-blue-700 font-medium flex items-center gap-1.5">
                  ...
                  {activeTechDetail.assignedWorkOrders.map((wo: any) => (
                    ...
                    >
                      <div>
                        ...
                        <div className="text-[11px] text-slate-500">{wo.status} &bull; Priority: {wo.priority} &bull; {wo.facilityName}</div>
                      </div>
