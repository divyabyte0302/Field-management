    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-4">
        <div>
          <div className="flex items-center space-x-2">
            ...
            <div>
              <h1 className="text-xl font-bold text-white tracking-wide font-mono">SLA MANAGEMENT & COMPLIANCE</h1>
              <p className="text-xs text-slate-400">Response & resolution deadline enforcement with automated escalation rules</p>
            </div>
          </div>
        </div>
      ...
      <div className="flex border-b border-slate-800 bg-slate-900/60 rounded-xl p-1 gap-1 text-xs font-semibold">
        ...
        <button
          ...
          className={`px-4 py-2 rounded-lg transition-all flex items-center gap-2 ${
            activeTab === 'breached'
              ? 'bg-rose-950/40 text-rose-400 shadow-sm border border-rose-800/40'
              : 'text-slate-400 hover:text-white'
          }`}
        >
          <AlertTriangle className="w-4 h-4 text-rose-400" />
          <span>Breached Work Orders</span>
          <span className="px-1.5 py-0.2 rounded-full text-[10px] bg-rose-900/60 text-rose-200 border border-rose-700">
        ...
        <button
          ...
          className={`px-4 py-2 rounded-lg transition-all flex items-center gap-2 ${
            activeTab === 'atRisk'
              ? 'bg-amber-950/40 text-amber-400 shadow-sm border border-amber-800/40'
              : 'text-slate-400 hover:text-white'
          }`}
        >
          <Clock className="w-4 h-4 text-amber-400" />
          <span>At-Risk Work Orders</span>
          <span className="px-1.5 py-0.2 rounded-full text-[10px] bg-amber-900/60 text-amber-200 border border-amber-700">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-slate-900 border border-slate-800 p-5 rounded-xl flex flex-col justify-between">
              <div className="flex items-center justify-between text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">
                ...
                <ShieldAlert className={`w-4 h-4 ${
                  (dashboardData?.compliancePercentage || 0) >= 90 ? 'text-emerald-400' :
                  (dashboardData?.compliancePercentage || 0) >= 75 ? 'text-amber-400' : 'text-rose-400'
                }`} />
              </div>
              <div className="flex items-baseline gap-2">
                ...
                <span className="text-[11px] text-slate-400">of active WOs meeting SLA</span>
              </div>
            ...
            <div className="bg-slate-900 border border-slate-800 p-5 rounded-xl flex flex-col justify-between">
              ...
              <div className="flex items-baseline gap-2">
                ...
                <span className="text-[11px] text-slate-400">orders in green SLA status</span>
              </div>
            ...
            <div className="bg-slate-900 border border-slate-800 p-5 rounded-xl flex flex-col justify-between">
              ...
              <div className="flex items-baseline gap-2">
                ...
                <span className="text-[11px] text-slate-400">approaching deadline</span>
              </div>
              <p className="text-[11px] text-amber-400/80 mt-3">Action needed: expedite technician dispatch or reassign</p>
            </div>
            ...
            <div className="bg-slate-900 border border-slate-800 p-5 rounded-xl flex flex-col justify-between">
              ...
              <div className="flex items-baseline gap-2">
                ...
                <span className="text-[11px] text-slate-400">deadline missed</span>
              </div>
              <p className="text-[11px] text-rose-400/80 mt-3">Requires priority escalation and root-cause audit</p>
            </div>
          </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 p-4 rounded-lg border border-slate-800">
                  <div className="text-[11px] text-slate-400 uppercase font-bold">Avg. Response Time</div>
                  <div className="text-2xl font-black text-cyan-300 font-mono mt-1">
                    {dashboardData?.averageResponseTimeMinutes || 24} min
                  </div>
                  <div className="text-[11px] text-slate-400 mt-1">
                    Compliance: <span className="text-emerald-400 font-bold">{dashboardData?.responseCompliantPercentage || 92}%</span>
                  </div>
                </div>

                <div className="bg-slate-950 p-4 rounded-lg border border-slate-800">
                  <div className="text-[11px] text-slate-400 uppercase font-bold">Avg. Resolution Time</div>
                  ...
                  <div className="text-[11px] text-slate-400 mt-1">
                    Compliance: <span className="text-emerald-400 font-bold">{dashboardData?.resolutionCompliantPercentage || 88}%</span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2 pt-2">
                <h4 className="text-xs font-bold text-slate-300 uppercase">Compliance by Priority Tier</h4>
                <div className="space-y-2">
                  {[
                    ...
                    <div key={tier.label} className="flex items-center justify-between text-xs bg-slate-950 p-2.5 rounded-lg border border-slate-800">
                      <div>
                      ...
                      <div className="flex items-center gap-3">
                        <div className="w-24 bg-slate-800 h-1.5 rounded-full overflow-hidden">
                          <div className={`h-full bg-cyan-400`} style={{ width: `${tier.pct}%` }} />
                        </div>
                        <span className="font-mono font-bold text-slate-300 w-10 text-right">{tier.pct}%</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-4">
              ...
              <div className="space-y-3">
                <div className="p-3 bg-slate-950 rounded-lg border border-slate-800 flex items-start gap-3">
                  ...
                  <div>
                    <div className="text-xs font-bold text-slate-200">Level 1: Dispatcher Reminder</div>
                ...
                <div className="p-3 bg-slate-950 rounded-lg border border-slate-800 flex items-start gap-3">
                  ...
                  <div>
                    <div className="text-xs font-bold text-slate-200">Level 2: Supervisor Escalation (AT_RISK)</div>
                    <p className="text-[11px] text-slate-400 mt-0.5">
                ...
                <div className="p-3 bg-slate-950 rounded-lg border border-slate-800 flex items-start gap-3">
                  <div className="w-8 h-8 rounded-lg bg-rose-500/10 text-rose-400 border border-rose-500/20 flex items-center justify-center font-bold text-xs shrink-0">
                    90%
                  </div>
                  <div>
                    <div className="text-xs font-bold text-slate-200">Level 3: Urgent Priority Page (BREACH PREVENT)</div>
                    <p className="text-[11px] text-slate-400 mt-0.5">
      ...
      {activeTab === 'breached' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between bg-rose-950/20 border border-rose-800/40 p-3 rounded-xl">
              ...
              <tbody className="divide-y divide-slate-800">
                {breachedOrders.length === 0 ? (
                  ...
                  breachedOrders.map(wo => (
                    <tr key={wo.id} className="hover:bg-slate-800/40 transition-all">
                      ...
                      <td className="py-3 px-4">
                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${
                          ...
                          wo.priority === 'HIGH' ? 'bg-amber-500/20 text-amber-400' : 'bg-blue-500/20 text-blue-400'
                        }`}>
                          {wo.priority}
                        </span>
                      </td>
                      <td className="py-3 px-4 text-slate-300">
                        {wo.assignedTechnicianName || <span className="text-slate-500 italic">Unassigned</span>}
                      </td>
                      <td className="py-3 px-4 text-rose-400 font-mono font-medium">
                        {new Date(wo.slaResolutionDeadline).toLocaleString()}
                      </td>
                      <td className="py-3 px-4">
                        <span className="px-2 py-0.5 bg-rose-500/20 text-rose-400 rounded font-bold font-mono text-[10px]">
                      ...
                      <td className="py-3 px-4 text-right">
                        {onSelectWorkOrder && (
                          <button
                            ...
                            className="px-2.5 py-1 bg-rose-600/80 hover:bg-rose-600 text-white rounded font-semibold text-[11px] inline-flex items-center gap-1"
                          >
                ...
                {atRiskOrders.length === 0 ? (
                  ...
                  atRiskOrders.map(wo => (
                    <tr key={wo.id} className="hover:bg-slate-800/40 transition-all">
                      ...
                      <td className="py-3 px-4 text-slate-200 max-w-[200px] truncate">
                        {wo.title}
                      </td>
                      ...
                      <td className="py-3 px-4">
                        <span className="px-2 py-0.5 bg-amber-500/20 text-amber-400 border border-amber-500/30 rounded font-bold font-mono text-[10px] inline-flex items-center gap-1">
                      ...
                      <td className="py-3 px-4 text-right">
                        {onSelectWorkOrder && (
                          <button
                            onClick={() => onSelectWorkOrder(wo)}
                            className="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 text-cyan-400 rounded font-semibold text-[11px] inline-flex items-center gap-1"
                          >
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="bg-slate-900 border border-slate-800 rounded-lg px-2.5 py-1 text-xs text-slate-200 font-medium"
              >
            ...
            {canManagePolicies && (
              <button
                onClick={handleOpenCreatePolicy}
                className="px-3 py-1.5 bg-cyan-600 hover:bg-cyan-500 text-white rounded-lg text-xs font-semibold flex items-center gap-1.5 shadow"
              >
                <Plus className="w-3.5 h-3.5" />
                Add Policy
              </button>
              ...
              >
                <div>
                  <div className="flex items-start justify-between gap-2">
                    <h4 className="text-sm font-bold text-white leading-snug">{policy.name}</h4>
                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase shrink-0 ${
                      policy.priority === 'CRITICAL' ? 'bg-rose-500/20 text-rose-400' :
                      policy.priority === 'HIGH' ? 'bg-amber-500/20 text-amber-400' :
                      policy.priority === 'MEDIUM' ? 'bg-blue-500/20 text-blue-400' : 'bg-slate-700 text-slate-300'
                    }`}>
                      {policy.priority}
                    </span>
                  </div>
                  ...
                  <div className="mt-3 text-[11px] text-slate-400 space-y-1">
                    <div>
                      <span className="text-slate-500">Facility Scope:</span>{' '}
                      <span className="text-slate-300">{policy.facilityName || 'All Facilities (Default)'}</span>
                    </div>
                    <div>
                      <span className="text-slate-500">Business Hours:</span>{' '}
                      <span className="text-slate-300">{policy.businessHoursOnly ? 'Yes (8am - 6pm M-F)' : '24/7 Global Clock'}</span>
                    </div>
                    <div>
                      <span className="text-slate-500">Escalation Stages:</span>{' '}
                      <span className="text-slate-300">{policy.escalationRules?.length || 3} Trigger Thresholds</span>
                    </div>
                  </div>
                </div>
                ...
                {canManagePolicies && (
                  <div className="pt-3 border-t border-slate-800 flex items-center justify-end gap-2 text-xs">
                    <button
                      ...
                      className="px-2.5 py-1 text-slate-300 hover:text-white bg-slate-800 hover:bg-slate-700 rounded font-semibold"
                    >
                      Edit
                    </button>
                    <button
                      onClick={() => handleDeletePolicy(policy.id, policy.name)}
                      className="px-2.5 py-1 text-rose-400 hover:text-rose-300 bg-rose-950/30 hover:bg-rose-950/50 rounded font-semibold"
                    >
                      Delete
                    </button>
                  </div>
          ...
          <div className="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-6 space-y-4">
            ...
            <form onSubmit={handleSavePolicySubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-300 mb-1">Facility Scope</label>
                  <select
              ...
              <div className="bg-slate-950 p-3 rounded-lg border border-slate-800 flex items-center justify-between">
                <div>
                  <div className="text-xs font-semibold text-slate-200">Business Hours Only</div>
