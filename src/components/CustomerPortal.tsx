  ...
  return (
    <div id="customer-portal-container" className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl text-white">
        <div>
          <div className="flex items-center gap-2 mb-1.5">
            <span className="px-2.5 py-0.5 rounded text-[10px] font-mono bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 font-bold uppercase">
              CLIENT PORTAL
            </span>
            <span className="text-xs text-slate-400 font-mono">• {user?.organizationId || 'Commercial Real Estate Tenant'}</span>
          </div>
          <h1 className="text-2xl font-bold text-white tracking-tight">Facility Maintenance & Service Hub</h1>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="py-2.5 px-4 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-semibold rounded-xl flex items-center gap-2 shadow-lg shadow-emerald-600/20 transition-all cursor-pointer"
          >
      ...
      {pendingVerificationOrders.length > 0 && (
        <div id="verification-banner" className="bg-cyan-950/40 border border-cyan-500/30 rounded-2xl p-5 shadow-lg">
          ...
          <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3.5">
            {pendingVerificationOrders.map(order => (
              <div key={order.id} className="bg-slate-900 border border-slate-800 rounded-xl p-4 flex flex-col justify-between space-y-3">
                <div>
                  <div className="flex items-center justify-between text-xs mb-1.5">
                    <span className="font-mono text-cyan-400 font-bold">{order.workOrderNumber}</span>
                    <span className="px-2 py-0.5 rounded bg-teal-500/10 text-teal-300 text-[10px] font-semibold border border-teal-500/20">
                      READY FOR SIGN-OFF
                    </span>
                  </div>
                  <h4 className="text-xs font-bold text-white">{order.title}</h4>
                  <p className="text-[11px] text-slate-400 mt-0.5">{order.facilityName} • Tech: {order.assignedTechnicianName || 'Field Tech'}</p>
                  {order.resolutionNotes && (
                    <div className="mt-2 p-2 rounded bg-slate-950/60 border border-slate-800/80 text-[11px] text-slate-300">
                      <span className="font-semibold text-slate-400">Resolution:</span> {order.resolutionNotes}
                    </div>
                  )}
                </div>
                <button
                  ...
                  className="w-full py-2 px-3 bg-cyan-600 hover:bg-cyan-500 text-white rounded-lg text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors cursor-pointer"
                >
      ...
      {showModal && (
        <div id="service-request-modal" className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-6 space-y-4 text-white">
            ...
            <form onSubmit={handleCreateRequest} className="space-y-3.5">
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">Facility Location</label>
                <select
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-300 mb-1">Location Details</label>
                  <input
              ...
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">Photo / Document Attachment URL (Optional)</label>
                <div className="flex gap-2">
                  ...
                  <input
                    ...
                    className="w-32 bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-slate-100 focus:outline-none focus:border-emerald-500"
                  />
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-6 space-y-4 text-white">
            ...
            <div className="p-3 bg-slate-950 rounded-xl border border-slate-800 text-xs space-y-1">
              <div className="font-mono font-bold text-cyan-400">{verifyingOrder.workOrderNumber}</div>
              <div className="font-semibold text-white">{verifyingOrder.title}</div>
              <div className="text-slate-400">{verifyingOrder.facilityName}</div>
              {verifyingOrder.resolutionNotes && (
                <div className="mt-2 text-slate-300 text-[11px]">
                  <span className="text-slate-400 font-semibold">Resolution:</span> {verifyingOrder.resolutionNotes}
                </div>
              )}
            </div>
